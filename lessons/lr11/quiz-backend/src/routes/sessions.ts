...
sessions.post('/', async c => { //эндпоинт, отправляем запрос 
  ...
  try {
    ...
    return c.json({ //если мы получили все перечисленные реквизиты, то мы должны вернуть айдишник статус вопросы, их количесвто И КОЛИЧЕСТВО ОТВЕЧЕННЫХ МЕНЯМ НА 0
