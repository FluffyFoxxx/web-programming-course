...
//9
//когда мы нажимаем на кнопку от бэка прилетаеет сессия, в которой хранятся вопросы. 
...
//если мы получаем от фронта запрос на то тчо в строке 53, то мы получаем обратно 10 вопросов 
...
//создаем в базе сессию и описываем ее реквизиты
...
//отправляем ответ 
  async submitAnswer(input: SubmitAnswerInput) {
    ...
    return await prisma.$transaction(async tx => {
      ...
      if (!session) throw new Error('Session not found') //если сессия не найдена 
      if (session.userId !== userId) throw new Error('Forbidden') //если не совпал айдишник 
      if (session.status !== 'active') throw new Error('Session is not active') //если неактивная сессия
      if (new Date() > session.expiresAt) { //сессия истекла
      ...
      const questionIds = session.questions as string[]  //проверка валидности ответа к вопросу (этот вопрос вообще был?) 
      if (!questionIds.includes(questionId)) {
        throw new Error('Question is not part of this session') 
      }

      const question = await tx.question.findUnique({ //ошибка что вопрос не найден
        where: { id: questionId },
      })
      if (!question) throw new Error('Question not found')

      const existing = await tx.answer.findUnique({ //ошибка что на вопрос уже ответили
        where: { sessionId_questionId: { sessionId, questionId } },
      })
      if (existing) throw new Error('Question already answered')

      if (question.type === 'multiple-select') { //проверяем что тут действительно может быть несколько вариантов
        if (!selectedOptions)
          throw new Error('selectedOptions required for multiple-select')

        const options = question.options as QuestionOption[] | null //ошибка отсутсвия ответов в ответах
        if (!options) throw new Error('Question has no options')

        const result = scoringService.scoreMultipleSelect( //описываем результат ответов мультичелектов
          options,
          selectedOptions,
          question.penaltyPerWrong ?? 0.5, //если в базе не указали вес, то будет половина прорка на отсутсвие -1
          question.minScore ?? 0,
        )

        const answer = await tx.answer.create({ //создаем ответ, передаем в него реквизиты
          data: {
            sessionId,
            questionId,
        ...
        return { //возвращаем все те же реквизиты
      ...
      } else {  //если вернуть не получилось, возвращаем ошиьбки
        ...
        return { //если все хорошо, то сохраняем в сессию
  ...
  async submitSession(sessionId: string, userId: string) { //Отправка сессии и проверки 
    return await prisma.$transaction(async tx => {
  ...
  async gradeEssay(input: GradeEssayInput, adminId: string) { // подсчет баллов 
    return await prisma.$transaction(async tx => {
